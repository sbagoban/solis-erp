...
// error was suppressed with the @-operator
    ...
    $paid_by = trim($_POST["transfer_paidBy"]);
    $payer = trim($_POST["transfer_payer"]);
	...
	if ($paid_by == 'TO') 
	{
        ...
        if ($transfer_type == 'ARR')
        {
            ...
            $qry_transferClaim->execute(array(":transfer_vehicle"=>$transfer_vehicle, ":service_name"=>$service_name,  ":special_name"=>$special_name,  ":arrivalDate"=>$arrivalDate, ":id_dept"=>$id_dept, ":payer"=>$payer));
        }
        else if ($transfer_type == 'DEP')
        {
        ...
        else if ($transfer_type == 'INTER HOTEL') 
        {
            $qry_serviceClaimAgency .= " WHERE  :departureDate BETWEEN claim_from AND claim_to";
            $qry_transferClaim = $con->prepare($qry_serviceClaimAgency);
            $qry_transferClaim->execute(array(":transfer_vehicle"=>$transfer_vehicle, ":service_name"=>$service_name,   ":departureDate"=>$departureDate, ":id_dept"=>$id_dept, ":payer"=>$payer));
        }
        else if ($transfer_type == 'ACTIVITY')
        {
            $qry_serviceClaimAgency .= " WHERE  :departureDate BETWEEN claim_from AND claim_to 
