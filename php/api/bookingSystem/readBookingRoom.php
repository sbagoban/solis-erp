...
try {
    ...
    if (!isset($_GET["id_booking_room_claim"])) {
        throw new Exception("INVALID ID". $_GET["id_booking_room_claim"]);
    }
    
    $id_booking_room_claim = $_GET["id_booking_room_claim"];
    ...
    $qryBookingRoom = $con->prepare("
            ...
            AND BR_CLAIM.id_booking_room_claim = C.id
            AND BR_CLAIM.id_booking_room_claim = :id_booking_room_claim
            AND BR_CLAIM.active = 1
            AND BR_COST.active = 1");
    $qryBookingRoom->execute(array(":id_booking_room_claim"=>$id_booking_room_claim));
