...
// error was suppressed with the @-operator
	...
	{
		$qry_Product_List = " 
		SELECT DISTINCT id_product_service,service_name FROM
		(
			(
				SELECT 
				...
				WHERE PS.id_product_service = PSC.id_product_service
   ...
   //$qry_Product_List .= " "."WHERE on_thursday = 1 AND ex_thursday = 0"; 
	...
	if ($paid_by == 'TO') 
	{
		$qry_activityProduct->execute(array(":activity_date"=>$activity_date, ":id_product"=>$id_product,":id_dept"=>$id_dept, ":id_country"=>$id_country, ":payer"=>$payer));
	}
	else if ($paid_by == 'Client') 
	{
		$qry_activityProduct->execute(array(":activity_date"=>$activity_date, ":id_product"=>$id_product,":id_dept"=>$id_dept));
	} 
	...
	if ($row_count_p > 0) 
	{
        ...
        $myData = $productServiceList;
        echo json_encode($myData);
    } 
	else 
	{
		$productServiceList[] = array(
				'id_product_service'	=> '0',
				'service_name'	=> 'NONE'
			);
